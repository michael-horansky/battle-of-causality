...
class gamemaster():
    ...
    def print_board_at_time(self, t, print_to_output = True):
    ...
    def print_board_horizontally(self):
        board_delim = ' | '
        total_board_lines = self.print_board_at_time(0, print_to_output = False)
        for t in range(1, self.T):
            cur_board_lines = self.print_board_at_time(t, print_to_output = False)
    ...
    def is_pos_available(self, t, pos, approach_azimuth, allow_box_pushing = True):
        if not self.is_board_symbol_available(self.get_square(pos)):
            return(False)
        if self.find_stone_at_position(t, pos, debug_msg = False) != -1:
            return(False)
        """
        if box at t, pos:
    ...
    def find_stone_at_position(self, t, pos, debug_msg = True):
        for faction in self.factions:
    ...
    def change_stone_position(self, t, pos0, pos1, a1 = -1):
    ...
    def move_stone(self, t, pos0, pos1, a1 = -1):
        ...
        for i in range(len(pos0)):
            self.change_stone_position(t+1, pos0[i], pos1[i], a1[i])
